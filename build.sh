...
echo "  sudo aws s3 cp s3://your-s3-bucket/cloudscale-backup.zip /tmp/plugin.zip && sudo rm -rf /var/www/html/wp-content/plugins/cloudscale-backup && sudo unzip -q /tmp/plugin.zip -d /var/www/html/wp-content/plugins/ && sudo chown -R apache:apache /var/www/html/wp-content/plugins/cloudscale-backup && php -r \"if(function_exists('opcache_reset'))opcache_reset();\""
